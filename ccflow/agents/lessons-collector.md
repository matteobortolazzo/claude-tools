...
## Process
1. Read `.claude/config.json` — check for `isMonorepo`
...
Read all `.claude/rules/*.md` files (headings + first section of each) to understand the available topics.
...
- If all file paths in the lesson fall within one project → append to `.claude/rules/lessons-learned-<slug>.md`
- If paths span multiple projects or are repo-level → append to `.claude/rules/lessons-learned.md`
